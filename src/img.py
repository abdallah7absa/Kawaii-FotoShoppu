...
def mean_filter(image_path, output_path, kernel_size=3):
    image = cv2.imread(image_path)
    if image is None:
    ...
    for i in range(k, height - k):
        for j in range(k, width - k):
            region = gray[i - k:i + k + 1, j - k:j + k + 1]
            mean = np.mean(region)
            filtered_image[i, j] = mean

    cv2.imwrite(output_path, filtered_image)
def weighted_filter(image_path, output_path):
    ...
    k = weighted_kernel.shape[0] // 2

    for i in range(k, height - k):
        for j in range(k, width - k):
            region = gray[i - k:i + k + 1, j - k:j + k + 1]
            weighted_sum = np.sum(region * weighted_kernel)
            weighted_sum = np.clip(weighted_sum, 0, 255)
            filtered_image[i, j] = weighted_sum

    cv2.imwrite(output_path, filtered_image)
def apply_kernel(image_path, output_path, kernel):
    image = cv2.imread(image_path)
    if image is None:
        raise ValueError("Image not found or unable to read")
    ...
    cv2.imwrite(output_path, filtered_image)
def point_detection(image_path, output_path):
    kernel = np.array([
        [ 0,  1,  0],
        [ 1, -4,  1],
        [ 0,  1,  0]
    ], dtype=np.float32)
    apply_kernel(image_path, output_path, kernel)
def horizontal_detection(image_path, output_path):
    kernel = np.array([
        [-1, -1, -1],
        [ 2,  2,  2],
        [-1, -1, -1]
    ...
    apply_kernel(image_path, output_path, kernel)
def vertical_detection(image_path, output_path):
    kernel = np.array([
        [-1,  2, -1],
        [-1,  2, -1],
        [-1,  2, -1]
    ...
    apply_kernel(image_path, output_path, kernel)
def diagonal_left_detection(image_path, output_path):
    kernel = np.array([
        [ 2, -1, -1],
        [-1,  2, -1],
        [-1, -1,  2]
    ...
    apply_kernel(image_path, output_path, kernel)
def diagonal_right_detection(image_path, output_path):
    kernel = np.array([
        [-1, -1,  2],
        [-1,  2, -1],
        [ 2, -1, -1]
...
def point_sharpening(image_path, output_path):
    ...
    apply_kernel(image_path, output_path, kernel)
def horizontal_sharpening(image_path, output_path):
    kernel = np.array([
        [-1, -1, -1],
        [ 2,  2,  2],
        [-1, -1, -1]
